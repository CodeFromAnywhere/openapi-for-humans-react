...
export const OpenapiOverviewPage = (props) => {
    ...
    const filteredOperations = search.trim() === ""
        ? openapiDetails.operations
        : openapiDetails.operations.filter((item) => item.id.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (_jsxs("div", { className: "p-20", children: [_jsx("h1", { className: "text-3xl", children: openapiDetails.document.info?.title }), _jsx("div", { className: "flex flex-row flex-wrap", children: links.map((link) => {
                    return (_jsx("a", { className: "pr-6 text-blue-500 hover:text-blue-600", href: link.url, children: link.title }, link.url));
                }) }), _jsx("div", { className: "my-10", children: _jsx(Markdown, { children: openapiDetails.document.info.description }) }), _jsx("p", { className: "", children: "Operations:" }), _jsx("input", { type: "text", placeholder: "Search", className: "p-2 m-2", value: search, onChange: (e) => setSearch(e.target.value) }), _jsx("ul", { className: "py-10", children: filteredOperations.map((item) => {
